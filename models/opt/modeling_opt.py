...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None, past_key_values_length: int = 0):
    """
    ...
    if past_key_values_length > 0:
        mask = torch.cat([torch.ones(bsz, past_key_values_length, dtype=dtype).to(mask.device), mask], dim=-1)
...
class OPTLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, attention_mask: torch.LongTensor, past_key_values_length: int = 0):
        ...
        positions = (torch.cumsum(attention_mask, dim=1).type_as(attention_mask) * attention_mask).long() + past_key_values_length - 1
...
PARALLELIZE_DOCSTRING = r"""
    ...
    Args:
        device_map (`Dict[int, list]`, optional, defaults to None):
            ...
            have fewer attention modules mapped to it than other devices.
"""
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def deparallelize(self):
        ...
        if self.project_in is not None:
            self.project_in = self.project_in.to("cpu")

        if self.project_out is not None:
            self.project_out = self.project_out.to("cpu")
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:
            ...
            expanded_attn_mask = _expand_mask(
                attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1], past_key_values_length=past_key_values_length
            )
        ...
        for idx, (decoder_layer, past_key_value) in enumerate(zip(self.layers, past_key_values)):
            ...
            if self.model_parallel:
                ...
                if past_key_value is not None:
                    past_key_value = tuple(past_state.to(hidden_states.device) for past_state in past_key_value)
