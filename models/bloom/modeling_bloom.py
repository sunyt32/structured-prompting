...
def build_alibi_tensor(attention_mask: torch.Tensor, num_heads: int, dtype: torch.dtype, past_key_values_length: int, prefix_parallel: int = None) -> torch.Tensor:
    """
    ...
    if prefix_parallel is not None and prefix_parallel > 1:
        ...
        arange_tensor = (arange_tensor * attention_mask)[:, None]
    else:
...
class BloomAttention(nn.Module):
    ...
    def forward(
        ...
        prefix_parallel: Optional[int] = None,
        ...
        if alibi.shape[0] != query_layer.shape[0]: # deepspeed tensor parallel
            num_attention_heads_per_partition = query_layer.shape[0]
            rank = dist.get_rank()
            alibi = alibi[rank * num_attention_heads_per_partition : (rank + 1) * num_attention_heads_per_partition]
        ...
        if prefix_parallel is not None:
            ...
            attention_probs = attn_weights / torch.sum(attn_weights, -1, keepdim=True)
...
class BloomModel(BloomPreTrainedModel):
    ...
    def forward(
        ...
        **deprecated_arguments
        ...
        alibi = build_alibi_tensor(attention_mask, self.num_heads, dtype=hidden_states.dtype, past_key_values_length=past_key_values_length, prefix_parallel=prefix_parallel)

        causal_mask = self._bloom_prepare_attn_mask(
            attention_mask,
